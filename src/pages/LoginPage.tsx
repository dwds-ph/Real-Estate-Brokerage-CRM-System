...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else if (firebaseErr.code === 'auth/email-already-in-use') setError('An account with this email already exists');
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 p-4">
      <div className="w-full max-w-md space-y-6 rounded-xl bg-white dark:bg-gray-950 p-8 shadow-lg">
        {/* Logo / Brand */}
        ...
        <button
          ...
          disabled={loading}
          className="w-full rounded-lg border bg-background px-4 py-2 text-sm font-medium hover:bg-muted disabled:opacity-50 flex items-center justify-center gap-2"
        >
        ...
        {/* Forgot Password / Toggle Register */}
        <div className="flex flex-col items-center gap-2 text-sm">
          {!isRegister && (
            <button onClick={handleResetPassword} className="text-primary hover:underline" disabled={loading}>
              Forgot password?
            </button>
          )}
          <button
