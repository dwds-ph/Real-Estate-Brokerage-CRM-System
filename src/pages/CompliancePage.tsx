...
import { subscribeComplianceChecklists, createChecklist, updateChecklist, getPHComplianceTemplate } from "@/services/complianceService";
...
export default function CompliancePage() {
  ...
  const [activeChecklist, setActiveChecklist] = useState<ComplianceChecklistType | null>(null);

  useEffect(() => {
    if (!brokerId) return;
    const unsub = onSnapshot(
      query(collection(db, "deals"), where("brokerId", "==", brokerId), orderBy("createdAt", "desc")),
  ...
  const handleToggle = useCallback(async (itemId: string) => {
    ...
    const updatedItems = activeChecklist.items.map((item) =>
      item.id === itemId ? { ...item, completed: !item.completed, completedAt: !item.completed ? Date.now() : undefined, completedBy: !item.completed ? userProfile?.id : undefined } : item,
    );
    ...
    await updateChecklist(activeChecklist.id, { items: updatedItems, progress });
  }, [activeChecklist, userProfile]);

  const handleUpdateNotes = useCallback(async (itemId: string, notes: string) => {
    if (!activeChecklist) return;
    const updatedItems = activeChecklist.items.map((item) => (item.id === itemId ? { ...item, notes } : item));
  ...
  return (
    <div className="space-y-4 max-w-2xl">
      ...
      <p className="text-sm text-muted-foreground">PH-specific deal closing compliance tracking</p>

      <div className="flex items-center gap-2">
        <label className="text-xs font-medium">Deal:</label>
        <select value={selectedDealId} onChange={(e) => setSelectedDealId(e.target.value)} className="flex-1 rounded-lg border bg-background px-3 py-1.5 text-sm">
      ...
      {selectedDealId && !activeChecklist && (
        <button onClick={handleCreateChecklist} className="w-full rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground">
