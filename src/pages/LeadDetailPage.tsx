...
const STATUS_OPTIONS = ['new', 'contacted', 'viewed', 'negotiating', 'closed', 'lost'] as const;
...
export default function LeadDetailPage() {
  ...
  const handleStatusChange = async (status: string) => {
    ...
    await updateDocById('leads', id, {
      ...
      activityTimeline: [
        ...
        { action: `Status changed to ${status}`, timestamp: Date.now(), by: userProfile?.displayName || 'Unknown' },
      ],
    });
  };
  ...
  const handleAddCommLog = async () => {
    ...
    await updateDocById('leads', id, {
      ...
      activityTimeline: [
        ...(lead.activityTimeline || []),
        { action: `${commType} logged: ${commText.trim().slice(0, 50)}${commText.length > 50 ? '...' : ''}`, timestamp: Date.now(), by: userProfile.displayName },
      ],
    });
    setCommText('');
    setSaving(false);
  ...
  return (
    <div className="space-y-6">
      {/* Back */}
      <button onClick={() => navigate('/leads')} className="text-sm text-muted-foreground hover:text-foreground">
        ← Back to Leads
      </button>
        ...
        <div className="flex items-start justify-between">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <span className={cn('rounded-full px-3 py-1 text-xs font-medium', getLeadStatusColor(lead.status))}>
                {lead.status}
              </span>
            ...
            <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
              ...
              {lead.propertyInterest && <span>🏠 Interested in: {lead.propertyInterest}</span>}
        ...
        <div className="rounded-lg border bg-card p-6">
          ...
          <div className="space-y-2 max-h-80 overflow-y-auto">
            {(!lead.communicationLog || lead.communicationLog.length === 0) ? (
              <p className="text-sm text-muted-foreground text-center py-4">No communication logged yet</p>
            ) : (
              [...lead.communicationLog].reverse().map((entry, i) => (
                <div key={i} className="rounded-lg bg-muted/50 p-3 text-sm">
                  <div className="flex items-center gap-2 text-xs text-muted-foreground mb-1">
                    <span>
                      {entry.type === 'call' ? '📞' : entry.type === 'text' ? '💬' : entry.type === 'meeting' ? '🤝' : '📧'}
                      {entry.type}
                    </span>
              ...
              [...lead.activityTimeline].reverse().map((entry, i) => (
                <div key={i} className="flex gap-3">
                  <div className="flex flex-col items-center">
                    ...
                    {i < lead.activityTimeline.length - 1 && <div className="w-px flex-1 bg-border" />}
                  </div>
                  <div className="pb-3">
                    <p className="text-sm">{entry.action}</p>
                    <p className="text-xs text-muted-foreground">{timeAgo(entry.timestamp)} by {entry.by}</p>
                  </div>
                </div>
              ))
            )}
