...
export default function CommissionsPage() {
  ...
  const commissionAmount =
    calcPrice && calcPercent
      ...
      : 0;
  const tax = commissionAmount
    ? calcTax(commissionAmount)
    : { vat: 0, withholding: 0 };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            ...
            {closedDeals
              .reduce((sum, d) => sum + (d as Deal).dealPrice, 0)
              .toLocaleString()}{" "}
            total volume
          </p>
        </div>
        ...
        <div className="rounded-lg border bg-card p-6 space-y-4 max-w-md">
          ...
          <div className="rounded-lg bg-muted p-4 space-y-2 text-sm">
            ...
            <div className="flex justify-between text-destructive">
              <span>VAT (12%)</span>
              <span>-{formatCurrency(tax.vat)}</span>
            </div>
            <div className="flex justify-between text-destructive">
              <span>Withholding Tax (1%)</span>
              <span>-{formatCurrency(tax.withholding)}</span>
            </div>
            <div className="flex justify-between border-t pt-2 font-bold">
              <span>Net Commission</span>
              <span>{formatCurrency(netCommission)}</span>
            </div>
          </div>
        </div>
      )}
                ...
                <div key={deal.id} className="rounded-lg border bg-card p-4">
                  ...
                  {deal.commission && (
                    <div className="mt-2 flex gap-4 text-xs text-muted-foreground border-t pt-2">
                      <span>
                        Total: {formatCurrency(deal.commission.total)}
                      </span>
                      <span>
                        Agent: {formatCurrency(deal.commission.agentShare)}
                      </span>
                      <span>
                        Broker: {formatCurrency(deal.commission.brokerShare)}
                      </span>
                      {deal.coBroking?.enabled && (
                        <span>
                          Co-broker:{" "}
                          {formatCurrency(deal.commission.agent2Share || 0)}
                        </span>
                      )}
                    </div>
                  )}
