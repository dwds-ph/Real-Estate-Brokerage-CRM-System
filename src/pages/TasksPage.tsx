...
export default function TasksPage() {
  ...
  const filtered = tasks
    ...
    .sort((a, b) => {
      const pa = (a as TaskItem).priority === 'high' ? 0 : (a as TaskItem).priority === 'medium' ? 1 : 2;
      const pb = (b as TaskItem).priority === 'high' ? 0 : (b as TaskItem).priority === 'medium' ? 1 : 2;
      return pa - pb;
    });
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">{tasks.filter((t) => (t as TaskItem).status === 'pending').length} pending</p>
        </div>
      ...
      <div className="flex gap-2">
        {(['pending', 'all', 'done'] as const).map((f) => (
          <button key={f} onClick={() => setFilter(f)} className={cn('rounded-full px-3 py-1 text-xs font-medium border capitalize', filter === f ? 'bg-primary text-primary-foreground border-primary' : 'bg-card hover:bg-muted')}>
            {f} ({tasks.filter((t) => f === 'all' || (t as TaskItem).status === f).length})
          </button>
        ))}
      </div>
        ...
        <form onSubmit={handleSubmit} className="rounded-lg border bg-card p-6 space-y-4 max-w-md">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={form.priority} onChange={(e) => setForm({ ...form, priority: e.target.value as TaskItem['priority'] })} className="w-full rounded-lg border bg-background px-3 py-2 text-sm">
      ...
      {loading ? (
        ...
        <div className="rounded-lg border bg-card p-8 text-center text-muted-foreground">No tasks</div>
      ) : (
        <div className="space-y-2">
          {filtered.map((t) => {
            const task = t as TaskItem;
            return (
              <div key={task.id} className={cn('rounded-lg border bg-card p-4 flex items-center gap-3', task.status === 'done' && 'opacity-60')}>
                <input
                  type="checkbox"
