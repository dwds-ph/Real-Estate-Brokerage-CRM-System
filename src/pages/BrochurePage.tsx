      ...
      <div className="mx-auto max-w-3xl px-4 py-8">
        ...
        <div className="rounded-xl overflow-hidden mb-6 shadow-lg">
          {listing.media && listing.media.length > 0 ? (
            <img src={listing.media[0]} alt={listing.title} className="w-full h-72 md:h-96 object-cover" />
          ) : (
        ...
        <div className="rounded-xl border bg-card p-6 md:p-8 shadow-sm space-y-6">
          <div className="flex items-start justify-between">
            <div>
              ...
              {listing.location?.city && (
                <p className="text-muted-foreground mt-1">📍 {listing.location.city}, {listing.location.province}</p>
              )}
            </div>
          ...
          <p className="text-3xl md:text-4xl font-bold text-primary">{formatCurrency(listing.price)}</p>
          ...
          <div className="border-t pt-6 text-center">
            ...
            <div className="flex justify-center gap-3">
              <a href={`https://wa.me/?text=I'm%20interested%20in%20${encodeURIComponent(listing.title)}`} target="_blank" rel="noopener noreferrer" className="rounded-lg bg-green-600 px-6 py-2 text-sm font-medium text-white hover:bg-green-700">
                💬 WhatsApp
              </a>
              <button onClick={() => window.print()} className="rounded-lg border px-6 py-2 text-sm font-medium hover:bg-muted">
                🖨️ Print
              </button>
            </div>
          </div>
