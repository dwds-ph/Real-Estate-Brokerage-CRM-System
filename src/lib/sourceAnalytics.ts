...
export function computeSourceAnalytics(leads: any[], deals: any[]): SourceAnalytics[] {
  const sourceMap = new Map<string, { leads: number; deals: number; commission: number; dealValues: number[]; leadTimestamps: number[]; dealTimestamps: number[] }>();

  for (const lead of leads) {
    const src = lead.source || "unknown";
    if (!sourceMap.has(src)) sourceMap.set(src, { leads: 0, deals: 0, commission: 0, dealValues: [], leadTimestamps: [], dealTimestamps: [] });
  ...
  return Array.from(sourceMap.entries()).map(([source, data]) => ({
    ...
    avgDealValue: data.dealValues.length > 0 ? data.dealValues.reduce((a, b) => a + b, 0) / data.dealValues.length : 0,
...
export function computeGoalProgress(goals: AgentGoal[], deals: any[]) {
  return goals.map((goal) => {
    const periodDeals = deals.filter((d) => {
      const created = d.createdAt || 0;
      return created >= goal.periodStart && created <= goal.periodEnd;
    });
    const dealsClosed = periodDeals.filter((d) => d.status === "closed" || d.status === "sold").length;
    const commission = periodDeals.reduce((sum, d) => sum + (d.commission || 0), 0);
    return {
      goal,
      dealsClosed,
      commission,
      dealProgress: goal.targetDeals > 0 ? dealsClosed / goal.targetDeals : 0,
      commissionProgress: goal.targetCommission > 0 ? commission / goal.targetCommission : 0,
    };
  });
}
