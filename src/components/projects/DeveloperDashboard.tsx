...
export default function DeveloperDashboard({
  ...
  const stats = useMemo(() => {
    const activeProjects = projects.filter(
      (p) => p.status === "ongoing" || p.status === "pre-selling",
    );
    const totalSold = units.filter((u) => u.status === "sold").length;
    const totalReserved = units.filter(
      (u) => u.status === "reserved" || u.status === "under-contract",
    ).length;
    ...
    const revenueByProject = projects
      .map((p) => {
        const projectUnits = units.filter(
          (u) => u.projectId === p.id && u.status === "sold",
        );
    ...
    return {
      ...
      sellThrough:
        units.length > 0 ? Math.round((totalSold / units.length) * 100) : 0,
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-lg border bg-card p-4">
        ...
        <div className="flex justify-between text-xs text-muted-foreground mt-1">
          ...
          <span>
            {stats.totalUnits - stats.totalSold - stats.totalReserved} available
          </span>
        </div>
      </div>
