...
export default function ProjectForm({ project, onSubmit, onClose }: ProjectFormProps) {
  ...
  const [developerContact, setDeveloperContact] = useState(project?.developerContact ?? "");
  ...
  const [projectType, setProjectType] = useState(project?.projectType ?? "subdivision");
  const [totalUnits, setTotalUnits] = useState(project?.totalUnits ?? 0);
  const [availableUnits, setAvailableUnits] = useState(project?.availableUnits ?? 0);
  ...
  const [commissionRate, setCommissionRate] = useState(project?.commissionRate ?? 0);
  const [amenities, setAmenities] = useState(project?.amenities.join(", ") ?? "");
      ...
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-card p-6 shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Developer Contact</label>
              <input
            ...
            <div>
              ...
              <select
                value={status}
                onChange={(e) => setStatus(e.target.value as "pre-selling" | "ongoing" | "completed" | "on-hold")}
                className="w-full rounded-md border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
              >
            ...
            <div>
              ...
              <select
                value={projectType}
                onChange={(e) => setProjectType(e.target.value as "subdivision" | "condo" | "commercial" | "mixed-use")}
                className="w-full rounded-md border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary"
              >
          ...
          <div>
            ...
            {phases.map((phase, i) => (
              <div key={phase.id} className="mb-2 rounded-md border bg-muted/30 p-3">
                <div className="flex items-center justify-between mb-2">
                ...
                <div className="grid grid-cols-4 gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) => handlePhaseChange(i, "totalUnits", Number(e.target.value))}
                    className="rounded-md border bg-background px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-primary"
                  />
                  <input
                    type="number"
                    ...
                    onChange={(e) => handlePhaseChange(i, "price.min", e.target.value)}
                    className="rounded-md border bg-background px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-primary"
                  />
                  <input
                    type="number"
                    ...
                    onChange={(e) => handlePhaseChange(i, "price.max", e.target.value)}
                    className="rounded-md border bg-background px-2 py-1 text-xs focus:outline-none focus:ring-1 focus:ring-primary"
                  />
                </div>
              </div>
