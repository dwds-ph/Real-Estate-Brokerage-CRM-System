...
export default function PropertyMap({
  ...
  const clusters = useMemo<ClusterResult>(() => {
    ...
    const filtered = geoListings.filter((l) => {
      ...
      if (
        ...
        return false;
      if (filters.statuses.length > 0 && !filters.statuses.includes(l.status))
        return false;
      if (
        filters.floodRisks.length > 0 &&
        !filters.floodRisks.includes(l.floodRisk)
      )
        return false;
  ...
  const toggleFilter = useCallback((key: keyof MapFilters, value: string) => {
    setFilters((prev) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {!singleMarker &&
          clusters.clusters.map((cl, i) => {
            ...
            const clusterIcon = createMarkerIcon(
              representative.status,
              representative.propertyType,
              true,
            );
            return (
              <Marker
                  ...
                  <div className="text-sm max-w-[200px] space-y-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {cl.listings
                        ...
                        .slice(0, 3)
                        .join(", ")}
                      {cl.listings.length > 3 &&
                        ` +${cl.listings.length - 3} more`}
                    </div>
                  </div>
                </Popup>
              </Marker>
            ...
            <div className="absolute top-12 right-3 z-[1000] rounded-lg border bg-card shadow-lg p-4 w-64 max-h-[60vh] overflow-y-auto space-y-3">
              ...
              <div className="border-t pt-2">
                <p className="text-xs text-muted-foreground">
                  {geoListings.filter((l) => l._geocoded).length}/
                  {geoListings.length} geocoded
      ...
      {!showLoading &&
        ...
        geoListings.every((l) => !l._geocoded) && (
          <div className="absolute inset-0 z-[1000] flex items-center justify-center bg-background/40">
            <div className="rounded-lg border bg-card p-6 text-center max-w-sm">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Addresses could not be found on the map. Check the location
