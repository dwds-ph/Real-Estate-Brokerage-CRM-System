...
export default function BranchDetail({ branch, agents, deals, onBack }: Props) {
  const branchAgents = agents.filter(
    (a: BranchAgent) =>
      a.officeId === branch.id || a.officeName === branch.name,
  );
  const branchDeals = deals.filter((d: BranchDeal) =>
    branchAgents.some(
      (a: BranchAgent) => a.id === d.agentId || a.id === d.createdBy,
    ),
  );
  const totalCommission = branchDeals.reduce(
    (s: number, d: BranchDeal) => s + (d.commission || 0),
    0,
  );
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-lg border bg-card p-4 space-y-3">
        <div className="flex items-start justify-between">
          ...
          <span
            className={`rounded-full px-2 py-0.5 text-[10px] ${branch.isActive ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
          >
