...
export default function LeadRoutingRules({ open, onClose }: Props) {
  ...
  const agentList = agents.filter(
    (a) => a.role === "agent" || a.role === "sub-agent",
  );
                ...
                <div key={i} className="rounded-lg border p-4 space-y-3">
                  ...
                  {rule.type === "round-robin" && (
                    <div className="space-y-2">
                      ...
                      <p className="text-[10px] text-muted-foreground">
                        Hold Ctrl/Cmd to select multiple. New leads cycle
                        through in order.
                      </p>
                    </div>
                  )}
                  ...
                  {rule.type === "specialty" && (
                    <div className="space-y-2">
                      ...
                      {rule.specialtyMap &&
                        Object.entries(rule.specialtyMap).length > 0 && (
                          <div className="space-y-1">
                            {Object.entries(rule.specialtyMap).map(
                              ([interest, agentId]) => {
                  ...
                  {rule.type === "location" && (
                    <div className="space-y-2">
                      <p className="text-xs text-muted-foreground">
                        Leads with matching location are auto-assigned.
                      </p>
                      <div className="flex gap-2">
                        <input
                          type="text"
                      ...
                      {rule.locationMap &&
                        Object.entries(rule.locationMap).length > 0 && (
                          <div className="space-y-1">
                            {Object.entries(rule.locationMap).map(
                              ([location, agentId]) => {
                                const agent = agentList.find(
                                  (a) => a.id === agentId,
                                );
