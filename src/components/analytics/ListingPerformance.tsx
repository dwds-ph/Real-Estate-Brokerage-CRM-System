...
export default function ListingPerformance({ listings, viewings, deals, loading }: ListingPerformanceProps) {
  const data = useMemo(() => {
    return listings.map((listing) => {
      ...
      const listingDeals = deals.filter(
      ...
      <div className="h-80">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            />
            <Legend />
            <Bar dataKey="Views" fill="#3B82F6" radius={[4, 4, 0, 0]} maxBarSize={20} />
            <Bar dataKey="Inquiries" fill="#F97316" radius={[4, 4, 0, 0]} maxBarSize={20} />
            <Bar dataKey="Viewings" fill="#A855F7" radius={[4, 4, 0, 0]} maxBarSize={20} />
          </BarChart>
        </ResponsiveContainer>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b text-left">
              ...
              <th className="pb-2 font-medium text-muted-foreground text-right">Inquiries</th>
              <th className="pb-2 font-medium text-muted-foreground text-right">Viewings</th>
              <th className="pb-2 font-medium text-muted-foreground text-right">Days on Market</th>
              <th className="pb-2 font-medium text-muted-foreground text-right">Converted</th>
            </tr>
          </thead>
          <tbody>
            {data.map((d) => (
              <tr key={d.fullTitle} className="border-b last:border-0 hover:bg-muted/50 transition-colors">
                ...
                <td className="py-2 text-right text-muted-foreground">{d['Days on Market']}d</td>
