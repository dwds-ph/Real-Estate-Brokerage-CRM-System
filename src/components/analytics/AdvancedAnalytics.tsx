...
export default function AdvancedAnalytics({ deals }: Props) {
  const activeDeals = deals.filter((d) => d.status !== "cancelled" && d.status !== "lost");
  const totalCommission = activeDeals.reduce((s, d) => s + (d.commission || 0), 0);
  const avgDealValue = activeDeals.length > 0 ? activeDeals.reduce((s, d) => s + (d.dealValue || 0), 0) / activeDeals.length : 0;
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="rounded-lg border bg-card p-3 text-center">
          ...
          <p className="text-xl font-bold text-primary">{formatCurrency(totalCommission)}</p>
        </div>
        <div className="rounded-lg border bg-card p-3 text-center">
          <p className="text-[10px] text-muted-foreground uppercase">Avg Deal Value</p>
          ...
          {months.map(([month, data]) => (
            <div key={month} className="space-y-1">
              <div className="flex justify-between text-xs">
                <span>{month}</span>
                <span className="text-muted-foreground">{data.deals} deals · {formatCurrency(data.commission)}</span>
              </div>
              <div className="h-3 rounded-full bg-muted overflow-hidden">
                <div className="h-full rounded-full bg-primary transition-all" style={{ width: `${(data.commission / maxCommission) * 100}%` }} />
              </div>
            </div>
          ))}
        </div>
