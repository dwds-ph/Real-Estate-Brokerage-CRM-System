...
export default function ExpenseVsCommission({
  ...
  const data = useMemo(() => {
    ...
    const filteredDeals = deals.filter(
      (d) => d.status === 'closed' && d.createdAt >= fromTs && d.createdAt <= toTs
    );

    const agentMap = new Map<string, AppUser>();
    agents.forEach((a) => agentMap.set(a.id, a));

    const agentData = new Map<string, { expenses: number; commission: number; name: string }>();
    ...
    <div className="space-y-6">
      ...
      <div className="h-80">
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
            ...
            <Bar dataKey="Commission" fill="#22C55E" radius={[4, 4, 0, 0]} maxBarSize={40} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {data.map((d) => {
              const roi = d.Expenses > 0
                ? ((d.Commission - d.Expenses) / d.Expenses * 100).toFixed(0)
                : '∞';
              return (
                <tr key={d.name} className="border-b last:border-0">
                  <td className="py-2 font-medium">{d.name}</td>
                  <td className="py-2 text-right text-red-600">{formatCurrency(d.Expenses)}</td>
                  <td className="py-2 text-right text-green-600">{formatCurrency(d.Commission)}</td>
                  <td className={`py-2 text-right font-medium ${d.Net >= 0 ? 'text-green-600' : 'text-red-600'}`}>
