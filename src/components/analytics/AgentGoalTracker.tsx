  ...
  return (
    <div className="rounded-lg border bg-card p-3 space-y-2">
        ...
        <div className="flex-1 rounded-full bg-muted overflow-hidden">
          <div
            className={cn(
              ...
              progress.dealProgress >= 1 && "bg-green-500",
            )}
            style={{ width: `${Math.min(progress.dealProgress * 100, 100)}%` }}
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 text-[10px] text-muted-foreground">
        <div>
          Deals: {progress.dealsClosed}/{goal.targetDeals}
        </div>
        <div>
          Commission: {formatCurrency(progress.commission)}/
          {formatCurrency(goal.targetCommission)}
        </div>
      </div>
    </div>
  );
