...
function createWrapper(initialEntries: string[] = ['/leads']): React.ComponentType<{ children: React.ReactNode }> {
  return ({ children }: { children: React.ReactNode }) =>
    React.createElement(MemoryRouter, { initialEntries } as MemoryRouterProps, children);
}
...
describe('useKeyboardShortcuts', () => {
  ...
  it('ignores key presses when target is INPUT', () => {
    ...
    const custom: ShortcutDef[] = [
      { keys: ['N'], label: 'N', description: 'New', action: { type: 'action', handler: customAction } },
    ];
  ...
  it('fires a single-key custom action shortcut', () => {
    const customAction = vi.fn();
    const custom: ShortcutDef[] = [
      { keys: ['N'], label: 'N', description: 'New lead', action: { type: 'action', handler: customAction } },
    ];
    renderShortcutHook(custom);
