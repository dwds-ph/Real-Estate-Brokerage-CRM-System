...
const DEFAULT_SHORTCUTS: ShortcutDef[] = [
  { keys: ['G', 'D'], label: 'G + D', description: 'Go to Dashboard', action: { type: 'navigate', path: '/dashboard' } },
  { keys: ['G', 'L'], label: 'G + L', description: 'Go to Leads', action: { type: 'navigate', path: '/leads' } },
  { keys: ['G', 'I'], label: 'G + I', description: 'Go to Listings', action: { type: 'navigate', path: '/listings' } },
  { keys: ['?'], label: '?', description: 'Toggle shortcuts help', action: { type: 'action', handler: () => {} } },
];
...
export function useKeyboardShortcuts(
  ...
  const helpShortcut = allShortcuts.find((s) => s.keys.length === 1 && s.keys[0] === '?');
  if (helpShortcut) {
    helpShortcut.action = { type: 'action', handler: () => setHelpOpen((p) => !p) };
  }
      ...
      for (const shortcut of allShortcuts) {
        ...
        if (shortcut.keys.length > 1) {
          ...
          if (allPressed) {
            ...
            if (shortcut.pagePattern && !shortcut.pagePattern.test(window.location.pathname)) {
              continue;
            }
            e.preventDefault();
            if (shortcut.action.type === 'navigate') {
