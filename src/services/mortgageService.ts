...
export const BANKS: BankProfile[] = [
  { id: 'bpi', name: 'BPI', typicalRate: '6.5% - 8.5%', estimatedTimelineDays: 45 },
  { id: 'bdo', name: 'BDO', typicalRate: '6.75% - 8.75%', estimatedTimelineDays: 50 },
  { id: 'metrobank', name: 'Metrobank', typicalRate: '6.5% - 8.5%', estimatedTimelineDays: 45 },
  { id: 'security-bank', name: 'Security Bank', typicalRate: '7.0% - 9.0%', estimatedTimelineDays: 55 },
  { id: 'eastwest', name: 'EastWest', typicalRate: '6.75% - 8.5%', estimatedTimelineDays: 40 },
];

// ─── Stage Definitions ─────────────────────────────────────────────

export const MORTGAGE_STAGES: { key: MortgageStage; label: string; description: string }[] = [
  { key: 'application', label: 'Application', description: 'Loan application submitted to bank' },
  { key: 'bank-evaluation', label: 'Bank Evaluation', description: 'Bank reviews documents and evaluates credit' },
  { key: 'bir-docs', label: 'BIR Docs', description: 'BIR document processing and stamping' },
  { key: 'rod', label: 'ROD', description: 'Registry of Deeds registration' },
  { key: 'loan-release', label: 'Loan Release', description: 'Loan proceeds released to seller/broker' },
];
...
export function getNextStage(currentStage: MortgageStage): MortgageStage | null {
  const currentIndex = STAGE_ORDER.indexOf(currentStage);
  if (currentIndex < 0 || currentIndex >= STAGE_ORDER.length - 1) return null;
  return STAGE_ORDER[currentIndex + 1];
}

export function getPreviousStage(currentStage: MortgageStage): MortgageStage | null {
  const currentIndex = STAGE_ORDER.indexOf(currentStage);
...
export async function fetchMortgagesByDeal(dealId: string): Promise<Mortgage[]> {
  const q = query(
  ...
  return snapshot.docs.map((d) => ({ id: d.id, ...d.data() } as unknown as Mortgage));
}
