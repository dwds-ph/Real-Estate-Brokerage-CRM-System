import { collection, query, where, orderBy, getDocs, addDoc, updateDoc, deleteDoc, doc, DocumentData } from 'firebase/firestore';
 ...
 * Create a new office.
 */
export async function createOffice(data: Omit<Office, 'id' | 'createdAt'>): Promise<string> {
 ...
 */
export async function getOfficeAgents(officeId: string): Promise<DocumentData[]> {
  const q = query(
    collection(db, 'users'),
